...
class cHTTPHeuristics(cPluginParent):
        ...
        for key in keys:
            if key != '':
                for ciphertext in ciphertexts:
                    ...
                    ciphertext = ciphertext.encode().decode('unicode-escape')
                    for iIter in range(len(ciphertext)):
                        # if key == 'Secret':
                        #     print (ciphertext, ciphertext[iIter])
    ...
    def Analyze(self):
        ...
        if resultHttp == []:
            resultHttp = [codecs.decode(line, 'rot-13') for line in self.Strings() if 'http:' in codecs.decode(line, 'rot-13').lower()]
        else:
            return resultHttp
