...
import zipfile
try:
    from StringIO import cStringIO ## for Python 2
except ImportError:
...
def HexDump(data):
    ...
    hexDump = ''
    for i, b in enumerate(str(data, 'ISO-8859-1')):
...
def SearchAndDecompressSub(data):
    ...
    if position == -1:
        ...
        compressedData = str(data[position - 3:],'ISO-8859-1')
...
def LoadPlugins(plugins, verbose):
    ...
    for plugin in sum(map(ProcessAt, plugins.split(',')), []):
        try:
            ...
            exec(open(plugin).read(), globals(), globals())
...
def OLEDump(filename, options):
    ...
    if filename == '':
        IfWIN32SetBinary(sys.stdin)
        oStringIO = BytesIO(sys.stdin.read())
    ...
    oStringIO.seek(0)
    if str(magic[0:4],'ISO-8859-1') == OLEFILE_MAGIC:
        ole = olefile.OleFileIO(oStringIO)
        returnCode = OLESub(ole, '', rules, options)
        ole.close()
    elif str(magic[0:2],'ISO-8859-1') == 'PK':
